...
export default function AdminTokensApp() {
  ...
  const [createTier, setCreateTier] = useState("start");
  ...
  async function createCode(e) {
    ...
    setCreateErr("");
    try {
      const res = await adminTokensApi.createCode(n, createTier);
      setLastCreated({
        code: res.code,
        expires_at: res.expires_at,
        days: res.days != null ? res.days : n,
        tier: res.tier || createTier,
      });
      setPage(0);
      await loadList();
    } catch (err) {
    ...
    <div className="adminTokensShell">
      ...
      <section className="adminTokensCard">
        ...
        <form className="adminTokensForm" onSubmit={createCode}>
          ...
          <div className="adminTokensField">
            <span className="adminTokensLabel">Тариф</span>
            <p className="adminTokensFieldHint">Привязывается к коду и передаётся пользователю при активации</p>
            <div className="adminTokensTierPills" role="group" aria-label="Тариф">
              {TIER_OPTIONS.map((opt) => {
                const isOn = createTier === opt.value;
                return (
                  <button
                    key={opt.value}
                    type="button"
                    className={`adminTokensTierPill${isOn ? " isActive" : ""}`}
                    onClick={() => setCreateTier(opt.value)}
                  >
                    {opt.label}
                  </button>
                );
          ...
          <div className="adminTokensCreated">
            ...
            <div className="adminTokensCreatedDetails">
              <p className="adminTokensCreatedLine">
                ...
                <span className="adminTokensCreatedVal adminTokensCreatedTier">
                  {String(lastCreated.tier || "start").toUpperCase()}
                </span>
              </p>
              <p className="adminTokensCreatedLine">
              ...
              return (
                <li key={row.code} className="adminTokensRow">
                  ...
                  <p className="adminTokensRowSummary">
                    ...
                    <span className="adminTokensRowTier">
                      {String(row.tier || "start").toUpperCase()}
                    </span>
                  </p>
